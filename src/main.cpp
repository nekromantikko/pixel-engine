...
int main(int argc, char** argv) {
...
#else
    // Headless mode - use system time for timing
    const s64 perfFreq = 1000000; // microseconds
    ...
    while (running) {
        ...
        // In headless mode, limit to reasonable frame rate to prevent busy loop
        std::this_thread::sleep_for(std::chrono::microseconds(16667)); // ~60 FPS
        
        // TODO: Add headless input injection mechanism here
        // For now, run for a limited time then exit
        static u32 headlessFrameCount = 0;
        headlessFrameCount++;
        if (headlessFrameCount > 3600) { // Run for ~60 seconds at 60 FPS
            running = false;
        }
#endif
